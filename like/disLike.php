<?php
	...
	*/
	...
	if(isset($_GET["id_me"]) && isset($_GET["id_comment"]))
	{
		...
		{
			$req = $bdd->prepare("DELETE like_comment  WHERE id_author=".$id_me." AND id_comment=".$id_comment.");");
			$result = $req->execute();
		...
		{
			$req = $bdd->prepare("DELETE like_post  WHERE id_author=".$id_me." AND id_post=".$id_post.");");
			$result = $req->execute();
